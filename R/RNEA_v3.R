...
# 		print(miRNA_counts[which(miRNA_counts[,3]!=0),]);
		...
		for(i in 1:length(which(miRNA_counts[,3]!=0))){
			...
			ResultsmiRNAtemp[i,1]<-phyper(miRNA_counts[r,3],Number_of_DE_genes,(miRNA_counts[r,2]-Number_of_DE_genes), miRNA_counts[r,1],lower.tail=FALSE);
			ResultsmiRNAtemp[i,2]<-phyper(miRNA_counts[r,4],Number_of_Up_genes,(miRNA_counts[r,2]-Number_of_Up_genes), miRNA_counts[r,1],lower.tail=FALSE);
			ResultsmiRNAtemp[i,3]<-phyper(miRNA_counts[r,5],Number_of_Down_genes,(miRNA_counts[r,2]-Number_of_Down_genes), miRNA_counts[r,1],lower.tail=FALSE);
		}
		ResultsmiRNA[,1]=p.adjust(ResultsmiRNAtemp[,1],method="fdr");
		ResultsmiRNA[,2]=p.adjust(ResultsmiRNAtemp[,2],method="fdr");
		ResultsmiRNA[,3]=p.adjust(ResultsmiRNAtemp[,3],method="fdr");
	}
...
# 		print(head(go_counts[which(go_counts[,3]!=0),]));
	...
	write.csv(unique(Network_final),file=paste(output,"_Network.csv", sep=""),quote=F,row.names=F);
	...
	if(type_of_output=="html"){
		...
		write.csv(as.data.frame(ResultsTF),file=paste(output,"_TF_Enrichment.csv", sep=""),row.names=F);
		write.csv(as.data.frame(ResultsmiRNA),file=paste(output,"_miRNA_Enrichment.csv", sep=""),row.names=F);
		write.csv(as.data.frame(Resultskegg),file=paste(output,"_KEGG_Enrichment.csv", sep=""),row.names=F);
		write.csv(as.data.frame(Resultskeggcat),file=paste(output,"_KEGG_category_Enrichment.csv", sep=""),row.names=F);
		write.csv(as.data.frame(Resultsgo),file=paste(output,"_GO_Enrichment.csv", sep=""),row.names=F);
	}
